...
#define WL_REG_ON 96 // WL_REG_ON is the input pin of WLAN module
#define WL_HOST_WAKE 98 // WL_HOST_WAKE is output pin of WLAN module
...
uint bcm_wlan_get_oob_irq(void)
{
	...
	if (gpio_request_one(wl_host_wake, GPIOF_IN, "bcmdhd") < 0) {
...
)
{
    ...
    if(err < 0){
        printk(KERN_ERR "wl_reg_on request failed.\n");
        return -1;
    }
